...
describe('Testing Detailed Card component - should correctly displays the detailed card data', async () => {
  ...
  test('shoud show correctly name', async () => {
    ...
    expect(name.textContent).toBe(mockCardResp.data.attributes.name);
  });
  ...
  test('shoud show correctly detail', async () => {
    ...
    expect(
      screen.getAllByTestId('DetailValue').filter((el) => el.textContent === 'heaven knows').length
    ).toBe(3);
  });
  ...
  test('shoud show correctly image', async () => {
    ...
    if (mockCardResp.data.attributes.image) {
      expect(image.src).toBe(mockCardResp.data.attributes.image);
    } else {

...
describe('Testing Card - should renders the relevant card data', async () => {
  ...
  test('should renders valid ico', async () => {
    ...
    expect(ico?.src).toBe(fakeRes.data[0].attributes.image);
  });

  test('should renders spare ico', async () => {
    const router = createMemoryRouter(routes, {
      initialEntries: ['/?page=4&limit=29'],
      initialIndex: 1,
    });
    ...
    expect(ico).instanceOf(Image);
    if (!(ico instanceof Image)) throw Error('Ico is not Image');
    expect(ico?.src).toMatch('3808217_cauldron_halloween_pot_potion_witch_icon');
  });
  ...
  test('should renders name', async () => {
    ...
    expect(name?.textContent).toBe(fakeRes.data[1].attributes.name);
  });
